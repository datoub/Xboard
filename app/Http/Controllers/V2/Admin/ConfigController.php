...
class ConfigController extends Controller
{
    ...
    private function getConfigMappings(): array
    {
        return [
            ...
            'email' => [
                ...
                'email_mass_rate_limit_enable' => (bool) admin_setting('email_mass_rate_limit_enable', 1),
                'email_mass_rate_limit_per_hour' => (int) admin_setting('email_mass_rate_limit_per_hour', 1800),
                'email_mass_dispatch_batch' => (int) admin_setting('email_mass_dispatch_batch', 50),
                'email_mass_retry_limit' => (int) admin_setting('email_mass_retry_limit', 3),
                'email_marketing_enable' => (bool) admin_setting('email_marketing_enable', false),
                'email_marketing_rules_json' => MailMarketingService::rulesForSettings(),
                ...$this->getMarketingRuleSettings(),
            ],
            'telegram' => [
    ...
    public function save(ConfigSave $request)
    {
        ...
        $this->normalizeMarketingRuleSettings($data);

        if (array_key_exists('email_marketing_rules_json', $data) && trim((string) $data['email_marketing_rules_json']) !== '') {
            json_decode((string) $data['email_marketing_rules_json'], true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                return $this->fail([422, '营销邮件规则JSON格式错误：' . json_last_error_msg()]);
            }
        }
        ...
        foreach ($data as $k => $v) {
            ...
            if (str_starts_with($k, 'email_marketing_') && $k !== 'email_marketing_enable' && $k !== 'email_marketing_rules_json') {
                continue;
            }
            if ($k == 'frontend_theme') {
    ...
    private function getMarketingRuleSettings(): array
    {
        $rules = app(MailMarketingService::class)->getRules();

        return [
            'email_marketing_register_no_order_enable' => (bool) ($rules['register_no_order_10m']['enabled'] ?? false),
            'email_marketing_register_no_order_minutes' => (int) ($rules['register_no_order_10m']['minutes'] ?? 10),
            'email_marketing_unpaid_order_enable' => (bool) ($rules['unpaid_order_10m']['enabled'] ?? false),
            'email_marketing_unpaid_order_minutes' => (int) ($rules['unpaid_order_10m']['minutes'] ?? 10),
            'email_marketing_expire_before_enable' => (bool) ($rules['expire_before_7d']['enabled'] ?? false),
            'email_marketing_expire_before_days' => (int) ($rules['expire_before_7d']['days'] ?? 7),
            'email_marketing_traffic_exhausted_enable' => (bool) ($rules['traffic_exhausted']['enabled'] ?? false),
            'email_marketing_expired_after_7d_enable' => (bool) ($rules['expired_after_7d']['enabled'] ?? false),
            'email_marketing_expired_after_15d_enable' => (bool) ($rules['expired_after_15d']['enabled'] ?? false),
        ];
    }

    private function normalizeMarketingRuleSettings(array &$data): void
    {
        $ruleKeys = [
            'email_marketing_register_no_order_enable',
            'email_marketing_register_no_order_minutes',
            'email_marketing_unpaid_order_enable',
            'email_marketing_unpaid_order_minutes',
            'email_marketing_expire_before_enable',
            'email_marketing_expire_before_days',
            'email_marketing_traffic_exhausted_enable',
            'email_marketing_expired_after_7d_enable',
            'email_marketing_expired_after_15d_enable',
        ];

        if (!array_intersect($ruleKeys, array_keys($data))) {
            return;
        }

        $rules = app(MailMarketingService::class)->getRules();
        $rules['register_no_order_10m']['enabled'] = (bool) ($data['email_marketing_register_no_order_enable'] ?? false);
        $rules['register_no_order_10m']['minutes'] = max(1, (int) ($data['email_marketing_register_no_order_minutes'] ?? 10));
        $rules['unpaid_order_10m']['enabled'] = (bool) ($data['email_marketing_unpaid_order_enable'] ?? false);
        $rules['unpaid_order_10m']['minutes'] = max(1, (int) ($data['email_marketing_unpaid_order_minutes'] ?? 10));
        $rules['expire_before_7d']['enabled'] = (bool) ($data['email_marketing_expire_before_enable'] ?? false);
        $rules['expire_before_7d']['days'] = max(2, (int) ($data['email_marketing_expire_before_days'] ?? 7));
        $rules['traffic_exhausted']['enabled'] = (bool) ($data['email_marketing_traffic_exhausted_enable'] ?? false);
        $rules['expired_after_7d']['enabled'] = (bool) ($data['email_marketing_expired_after_7d_enable'] ?? false);
        $rules['expired_after_15d']['enabled'] = (bool) ($data['email_marketing_expired_after_15d_enable'] ?? false);
        ...
        $data['email_marketing_rules_json'] = json_encode($rules, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    }
