...
class UserController extends Controller
{
    ...
    public function sendMail(UserSendMail $request)
    {
        ...
        $rateLimitEnabled = (bool) admin_setting('email_mass_rate_limit_enable', 1);
        $rateLimitPerHour = $rateLimitEnabled
        ...
        $campaign = MailCampaign::create([
            ...
            'scope_payload' => [
                ...
                'filter' => $scope === 'filtered' ? $request->input('filter', []) : [],
                'sort' => $sort,
                'sort_type' => $sortType,
            ],
        ...
        $builder->chunk($chunkSize, function ($users) use ($campaign, $appName, $appUrl, &$total) {
            $rows = [];
            foreach ($users as $user) {
                $vars = [
                    'app.name' => $appName,
            ...
            if ($rows) {
                DB::table('v2_mail_campaign_recipients')->insertOrIgnore($rows);
                $total += count($rows);
            }
        });

        $actualTotal = MailCampaignRecipient::where('campaign_id', $campaign->id)->count();
        $campaign->total_count = $actualTotal;
        $campaign->queued_count = $actualTotal;
        $campaign->save();
        ...
        return $this->success([
            ...
            'estimated_hours' => $campaign->rate_limit_per_hour > 0 ? round($actualTotal / $campaign->rate_limit_per_hour, 2) : null,
            'message' => '群发任务已创建，将按限速在后台持续发送',
        ]);
    }

    public function mailCampaigns(Request $request)
    {
        $perPage = min(100, max(10, (int) $request->input('per_page', 20)));
        $campaigns = MailCampaign::orderBy('id', 'desc')->paginate($perPage);
        return $this->success($campaigns);
    }

    public function mailCampaignDetail(Request $request)
    {
        $campaign = MailCampaign::find((int) $request->input('id'));
        if (!$campaign) {
            return $this->fail([404, '群发任务不存在']);
        }
        $campaign->recipients_summary = MailCampaignRecipient::where('campaign_id', $campaign->id)
            ->select('status', DB::raw('count(*) as total'))
            ->groupBy('status')
            ->pluck('total', 'status');
        $campaign->recent_errors = MailCampaignRecipient::where('campaign_id', $campaign->id)
            ->whereNotNull('last_error')
            ->orderBy('updated_at', 'desc')
            ->limit(20)
            ->get(['email', 'status', 'attempts', 'last_error', 'updated_at']);
        return $this->success($campaign);
    }

    public function pauseMailCampaign(Request $request)
    {
        $campaign = MailCampaign::find((int) $request->input('id'));
        if (!$campaign) {
            return $this->fail([404, '群发任务不存在']);
    ...
    public function resumeMailCampaign(Request $request)
    {
        $campaign = MailCampaign::find((int) $request->input('id'));
        if (!$campaign) {
            return $this->fail([404, '群发任务不存在']);
        }
        if (in_array($campaign->status, ['paused', 'paused_by_system', 'pending', 'sending'], true)) {
    ...
    public function cancelMailCampaign(Request $request)
    {
        $campaign = MailCampaign::find((int) $request->input('id'));
        if (!$campaign) {
            return $this->fail([404, '群发任务不存在']);
