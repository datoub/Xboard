    ...
    "site": {
      ...
      "form": {
        ...
        "siteUrl": {
          ...
          "description": "兼容旧版本的默认网址。建议新增配置后，把节点/API 入口填到后端对接域名，把用户访问入口填到用户前端域名。"
        },
        "backendUrl": {
          "label": "后端对接域名",
          "placeholder": "例如：https://xbo.example.com",
          "description": "节点安装、节点通信、支付异步通知等后端/API 场景优先使用该域名。留空则回退到站点网址。"
        },
        "frontendUrl": {
          "label": "用户前端域名",
          "placeholder": "例如：https://www.example.com",
          "description": "用户主题、地址发布页、指定支付回跳等用户访问场景使用该域名。留空则回退到站点网址。"
        },
        "paymentReturnMode": {
          "label": "支付回跳方式",
          "placeholder": "请选择支付完成后的回跳域名",
          "description": "选择实际访问域名时，会优先回到用户下单时访问的域名；选择指定前端域名时，固定回到用户前端域名。",
          "options": {
            "source": "回到用户实际访问域名",
            "frontend": "固定回到用户前端域名"
          }
    ...
    "dynamic_form": {
      ...
      "shadowsocks": {
        ...
        "accept_proxy_protocol": {
          ...
          "description": "仅当入口转发会发送 PROXY Protocol v1/v2 头时开启；直连节点保持关闭"
        },
